...
- [ ] 5.9 Verify Cloudflare Pages project + API token exist; `wrangler pages deploy` succeeds — **blocked on owner** (requires the owner's Cloudflare account and `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` secrets)
- [ ] 5.10 Confirm `hcd@fragua.dev` mailbox receipt before marking escalation done — **blocked on owner** (depends on 5.2)
- [ ] 5.11 Verify `https://ordenanzas.fragua.dev` resolves BEFORE the full 1,038-document import (task 3.5). It is the contact URL in the crawler's `User-Agent`, so until the site is live an HCD administrator reading their logs follows it to nothing; `hcd@fragua.dev` is the only working channel until then — **blocked on owner** (requires the owner's DNS/Cloudflare Pages account; note task 3.5 has already landed per the current corpus import, so this verification is now retroactive/overdue rather than a pre-condition)
