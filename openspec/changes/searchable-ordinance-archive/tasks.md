...
- [x] 5.0 Add a favicon: `/buscar` currently 404s on `/favicon.ico`. Cosmetic, but it is a request every visitor's browser makes. `site/public/favicon.svg` — a self-contained inline document glyph, no external request, no municipal insignia (this archive is deliberately unofficial) — linked from `Layout.astro`'s `<head>`

- [x] 5.1 `pipeline/src/hcd_sync/notify.py`: `send_alert(message, transport=...)` POSTs to the Resend HTTP API (`bot@alerts.fragua.dev` → `hcd@fragua.dev`), reads `RESEND_API_KEY` from `os.environ` only, never raises (a delivery failure is logged and swallowed so it can never mask the job's own non-zero exit), and on any error logs the HTTP status or exception only — never the key, headers or body. Wired as `cli.main`'s `notifier=notify.send_alert` for the `run` subcommand. TEST `tests/test_notify.py` (5 cases, including a `caplog`-based assertion that the literal key substring never reaches a log record across the transport-error, provider-error-status and missing-key paths)
- [x] 5.2 Resend verified on `fragua.dev` itself (not the `alerts.` subdomain D6 proposed — Resend isolates its return-path in `send.fragua.dev` and never touches the apex SPF, so one verified domain suffices). A live alert was sent 2026-08-05 through the production `send_alert` via `.github/workflows/verify-escalation.yml`: **Resend accepted, HTTP 200**, from `bot@fragua.dev` to `hcd@fragua.dev` — **blocked on owner** (requires the owner's Cloudflare account)
- [x] 5.3 `.github/workflows/sync-and-deploy.yml`: job `sync` (`permissions: contents:write`, top-level `concurrency: { group: sync, cancel-in-progress: false }`) runs `hcd-sync run`, commits via `.github/scripts/commit-and-push-data.sh`, then fails the job loudly on a non-zero sync exit code (the unconditional fail-safe layer). Job `build-deploy` (`needs: sync`, `if: always()`) checks out `ref: main` explicitly (D3 gotcha #2) and deploys `site/dist` to Cloudflare Pages
- [x] 5.4 TEST `tests/test_sync_commit_push.py::test_no_change_sync_commits_nothing_and_exits_zero`: two no-change syncs against a real local git repo → zero commits, exit 0. Companion test confirms a change under `data/` is committed while a concurrent change outside `data/` stays unstaged (`git add data/` only)
- [x] 5.5 TEST `tests/test_sync_commit_push.py::test_non_fast_forward_push_retries_once_via_rebase_then_fails_loudly`: a genuine conflicting concurrent push forces the single `git pull --rebase` retry into a real conflict → non-zero exit, not an infinite retry loop and not a force-push
- [x] 5.6 `.github/workflows/ci.yml`: on `pull_request`/`push: [main]`, runs the exact `test_command` and `build_command` from `AGENTS.md`, then `actionlint` via `reviewdog/action-actionlint@v1`
- [x] 5.7 TEST `tests/test_workflows.py`: `actionlint` passes for real when installed (skipped, not faked, otherwise); a line-scanning check asserts no `${{ }}` template expression appears inside any `run:` block in any workflow — verified to actually catch the pattern by re-introducing it and confirming RED, then restoring the fix (GREEN)
- [x] 5.8 Repo-root `README.md`: layout table, the D1 Astro-not-Next.js deviation recorded in full (payload/Pagefind rationale, cost accepted, overrulable at zero design cost), non-negotiables summary, dev commands, operations/escalation section
- [~] 5.9 Cloudflare Pages project `ordenanzas-hcd` CREATED 2026-08-05 (production branch `main`, will serve at ordenanzas-hcd.pages.dev). Still needed: the `CLOUDFLARE_API_TOKEN` (scope `Cloudflare Pages: Edit`) and `CLOUDFLARE_ACCOUNT_ID` repo secrets, and a real `wrangler pages deploy`. **The first deploy is deliberately deferred until the visual design lands** — publishing puts 1,038 municipal documents on an indexable public URL under Fragua's name, and the first impression should not be an unstyled site — **blocked on owner** (requires the owner's Cloudflare account and `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID` secrets)
- [~] 5.10 Mailbox receipt: the owner must confirm the 2026-08-05 test alert ARRIVED. API acceptance is not delivery — the Email Routing rule for `hcd@` is the half Resend does not control, and an alert that is accepted but never lands is worse than none, because it reads as working — **blocked on owner** (depends on 5.2)
- [x] 5.11 DNS: DONE — `ordenanzas.fragua.dev` resolves to Cloudflare with a valid certificate (Google Trust Services, issued 2026-08-05). HTTP 522 until the first deploy, which is deliberate.
      Original note: the zone is on Cloudflare, the site itself is on Vercel, and there is no wildcard. The record cannot meaningfully exist before the host — the correct path is Pages project → Custom domains → `ordenanzas.fragua.dev`, which makes Cloudflare create the CNAME itself. Project created; the custom domain is the owner's dashboard step. Originally worded as a precondition for task 3.5; that import has already run, so this is now retroactive. Verify `https://ordenanzas.fragua.dev` resolves. It is the contact URL in the crawler's `User-Agent`, so until the site is live an HCD administrator reading their logs follows it to nothing; `hcd@fragua.dev` is the only working channel until then — **blocked on owner** (requires the owner's DNS/Cloudflare Pages account; note task 3.5 has already landed per the current corpus import, so this verification is now retroactive/overdue rather than a pre-condition)
