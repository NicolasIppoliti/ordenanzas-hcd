...
- [ ] 5.2 Confirm/create Cloudflare Email Routing rule `hcd@fragua.dev`; send + confirm one live test alert — **blocked on owner** (requires the owner's Cloudflare account)
...
- [ ] 5.10 Confirm `hcd@fragua.dev` mailbox receipt before marking escalation done — **blocked on owner** (depends on 5.2)
- [~] 5.11 DNS: the zone is on Cloudflare, the site itself is on Vercel, and there is no wildcard. The record cannot meaningfully exist before the host — the correct path is Pages project → Custom domains → `ordenanzas.fragua.dev`, which makes Cloudflare create the CNAME itself. Project created; the custom domain is the owner's dashboard step. Originally worded as a precondition for task 3.5; that import has already run, so this is now retroactive. Verify `https://ordenanzas.fragua.dev` resolves. It is the contact URL in the crawler's `User-Agent`, so until the site is live an HCD administrator reading their logs follows it to nothing; `hcd@fragua.dev` is the only working channel until then — **blocked on owner** (requires the owner's DNS/Cloudflare Pages account; note task 3.5 has already landed per the current corpus import, so this verification is now retroactive/overdue rather than a pre-condition)
